  ...
  public function listProducts( $mData, $iList = null, $bPagination = true ){
    ...
    if( isset( $aProducts ) && is_array( $aProducts ) ){
      ...
      for( $i = $aKeys['iStart']; $i < $aKeys['iEnd']; $i++ ){
        ...
        $content .= '<li class="pure-u-1-2 l'.( ( $i == ( $aKeys['iEnd'] - 1 ) ) ? 'L': $i2 + 1 ).' i'.( ( $i % 2 ) ? 0: 1 ).' column'.( ( $i % 3 ) ? 0: 1 ).'">
          <h2 class="product-category-title pure-u-1"><a href="'.$aData['sLinkName'].'">'.$aData['sName'].'</a></h2>
